 ...
 * the Gateway over HTTP, and for self-hosted users who prefer HTTP over CLI.
 *
...
import { parseJsonFromCliOutput, type RunCliResult } from "../cli";
import type { OpenClawTransport, TransportMode } from "../transport-types";

export class HttpTransport implements OpenClawTransport {
  ...
  private async invoke<T>(
    ...
    try {
      ...
      if (body && typeof body === "object" && "ok" in body) {
        if (body.ok === false) {
          throw new Error((body as { error?: { message?: string } }).error?.message || `Tool ${tool} failed`);
        }
  ...
  async readFile(path: string): Promise<string> {
    ...
    return this.resultToText(result as { output?: string; details?: unknown });
  }
